...
directory (with `main.tex`) **and** a working `script/latex/<name>/build.sh`.
...
each with `main.tex`, `sections/`, `references.bib` and a matching `script/latex/<id>/build.sh`.
...
- **E2E:** `./scripts/start_web.sh`, open `http://localhost:5173`, confirm the dropdown is
...
- **Edge:** rename a `script/latex/<id>/build.sh`; confirm that template appears disabled in
...
- Authoring new template directories (a new template is simply a `latex/<id>/` dir + a
  `script/latex/<id>/build.sh`; once added, it appears automatically via the registry).
