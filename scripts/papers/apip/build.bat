...
rem
rem  Windows batch equivalent of scripts/papers/apip/build.sh.
rem
rem  The paper is "Your Agent is Underperforming: The Case for Agent
rem  Performance Improvement Plans", converted from
rem  papers\apip\manuscript\APIP_Paper_v8_tracked.docx into the IEEE
rem  conference style.
rem
rem  Usage (from anywhere -- paths resolve relative to this script):
rem      scripts\papers\apip\build.bat
rem
rem  Options:
...
set "SRC_DIR=%REPO_ROOT%\papers\apip\latex"
set "CLEAN=0"
set "QUIET=0"
...
if not exist "%SRC_DIR%\main.tex" (
    ...
    echo         Expected the APIP paper at papers\apip\latex\, or pass another
...
:usage
echo build.bat -- Compile the APIP paper ^(papers\apip\latex^) to PDF
echo.
echo Usage:
echo     scripts\papers\apip\build.bat [options]
echo.
echo Options:
...
echo                       ^(default: papers\apip\latex^).
echo     -h, --help, /?    Show this help text.
echo.
echo Requirements:
echo     pdflatex and bibtex on PATH ^(MiKTeX or TeX Live for Windows^).
echo     IEEEtran.cls and IEEEtran.bst are vendored in papers\apip\latex\, so no
