...
for end-user docs see `MANUAL.md`, `README.md`, and `DEPLOY-AWS.md`.
...
- **Working tree:** `CRITIQUE_SPEC_5.md` is untracked (a design note added outside this session).
...
5. **The Critic** — `src/critic/`: per-section 3 criticisms + AI-genericness score (hybrid
   ...
   `POST /critic/adhoc`. Built from `SPEC-critic-button.md`. `d89cd2b`
6. **AWS deployment** — `docker/backend.Dockerfile` (Python + TeX Live), `web/Dockerfile`,
   `.github/workflows/deploy-aws.yml` (OIDC → ECR → ECS Express Mode), `DEPLOY-AWS.md`. `0707c56`
...
9. **`MANUAL.md`** — full user guide. `1d569b5`
10. **Reference extraction** — `src/docbuilder/refextract.py` + `references_warning` on the
    document model/response/UI (`CRITIQUE_SPEC.md` line of work). Recent commits.
11. **PowerPoint deck builder** — `build_deck.py` generates a `.pptx` from `POWERPOINT_SPEC.md`.
    Recent commits.

## How to run / test / deploy
```bash
uv sync                       # deps (add --extra llm for the Claude critic)
./scripts/start_web.sh        # backend :8000 + React dev :5173   (stop_web.sh to stop)
python main.py                # backend only (single entry point)
uv run --group dev pytest     # tests
npm run build                 # in web/ — type-check + build the frontend
```
Deploy: follow `DEPLOY-AWS.md` (ECR + two ECS roles + OIDC role → set GitHub vars/secrets → push
...
- Product: `src/docbuilder` (converter, CLI, sandbox, templates, refextract), `src/docserver`
  (FastAPI), `src/critic`, `web/` (React/Vite), `latex/` + `script/latex/` (templates/builds).
- **src layout, no build backend** — run with `PYTHONPATH=src` (tests set it via
  `pyproject.toml`; `main.py` and `scripts/start_web.sh` set it).
...
- Key env vars: `DOCSERVER_API_KEY`, `DOCSERVER_CORS_ORIGINS`, `LATEX_SANDBOX`, `LATEX_BUILD_TIMEOUT`,
  `LATEX_AUTO_INSTALL`, `CRITIC_PROVIDER`, `ANTHROPIC_API_KEY` (see `MANUAL.md` §7).

## Open items / suggested next steps
- **Push** the unpushed commits on `devel-apip-fullfeatures`; consider a PR → `main`.
- Review/triage the `CRITIQUE_SPEC*.md` notes (1–5) and reconcile with the implemented Critic.
...
- A tailored `POWERPOINT_SPEC.md` was created in **16 top-level `fable5` projects** (create-only,
