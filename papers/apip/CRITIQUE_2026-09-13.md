...
The ChatGPT pass changed the shape of the endgame in a useful way: **four of its five items are
rewordings, not additions**, so they fit inside a page budget that has no room. They are now the
cheapest substantive edits available.

1. **The wording pass — M10, D14, D15, M11.** All four are swaps or single sentences, and
   together they repair the paper's conceptual spine: what a tier *is* (M10), saying so before
   Table I uses it (D14), not calling a hyperparameter "the cost of diagnosis" (D15), and saying
   what "durable" was measured over (M11 — 11 ticks for the τ² cell, 12 for ChatDev creep, both
   already in the records). Do these together; they interlock.
2. **M9 — the taxonomy precedence rule.** A wording change too, and the most valuable of the
   five: it is the defect that produced D12, and it makes C4's confusable cases well-defined
   before anyone tests them.
3. **P2 or P1 — the page cut.** Still the author's manual pass, still the gate on anything
   additive.
4. **D13, M7's citation, AgentTether** — the bibliography pass, once there is room.

**Out of reach before the deadline:** C3, C4, M1 (the replication attempt failed for reasons now
recorded in §V-E), M2, M3, M4, D2--D7. These are 12-page items.

**Free, five minutes each:** P5, P6, P7.

**What today established without any experiment:** C2, C5, D10, D11, D12 and C1's evidence all
came from files already on disk. The one thing compute produced was a negative — no available
backbone can reproduce the reported cells — and that is now the §V-E limitation rather than a
gap.
