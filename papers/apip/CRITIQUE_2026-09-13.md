...
*Status: **new**, from the 2026 literature check. Extends M2.*
...
- **SafeHarness: Lifecycle-Integrated Security Architecture for LLM-based Agent Deployment**
  (Lin et al., arXiv:2604.13630v2, May 2026 — **title and authors verified; its tier structure
  is NOT verified**, the summary I have of it is inferred rather than quoted). Reported to
  escalate verification tiers on a windowed violation rate. Read it before citing.
- **AgentTether** (arXiv:2607.06273) and Bayesian self-escalation work (arXiv:2608.24087) —
...
*Status: **new**, verified against the paper and the frozen mapping table.*
...
**It also blocks the C3 experiment.** A stale-tool-schema injection run under the frozen
mapping selects a prompt patch for a schema problem, then escalates to fine-tuning. That is
not "Tier 2 repairs the mechanism"; it is APIP mis-tiering on camera. Decide the mapping
before spending compute, and note that the mapping table cannot simply be edited — its hash
is the pre-registration anchor, so a change means a documented v2 and a re-run, not a quiet
fix.
...
There is a version of this worth keeping. If the frozen mapping stands, the experiment tests
**escalation** — Tier 1 mis-selects, escalation to Tier 3 recovers — and escalation has never
been exercised: `escalated_from` is null in every seed-13 record, and D5 already notes only
two of five exit states have been observed. "The protocol recovers from its own mis-tiering"
is a stronger claim than "the right tier works."
...
| **M7** | **(needs space)** Two moves. Cite Qi et al. for shallow alignment in §II or §IV-C, and use it in §V-C to explain the s3/s4 split instead of leaving "necessary, not sufficient" hanging. Then decide whether Table I's creep→Tier 1 row survives: if prompt-depth fixes are shallow by mechanism, the row needs a second tier or an escalation note. The §V-C half is the valuable one and is nearly word-neutral if it replaces the existing sentence. |
| **M8** | **(needs space)** Read RAIL Guard, then add one sentence to §II-A citing it and naming the difference: dimension-level content remediation versus a cause-attributed lifecycle with a bounded window and durability test. Read SafeHarness before deciding whether it needs the same. Do this before submission regardless of page budget — it is the cheapest insurance against a novelty rejection. |
| **D12** | **(fits — it is a deletion)** Decide which is right and make the other agree. The literature and common sense favour §IV-C: schema defects are context-layer. If Tier 2 wins, Table I's row changes and the mapping table needs a documented v2 plus a re-run of anything that cites the old hash. If Tier 1/3 wins, delete the tool-misuse clause from §IV-C's Tier 2 sentence — a four-word cut, and the cheapest resolution. Either way, settle it before the C3 experiment. |
...
The literature pass changed the ordering again. Two of the new items are cheap and urgent,
and one of them gates the experiment that was going to be run tonight.

1. **D12 — settle the tool-misuse tier, before any compute.** Table I and the frozen mapping
   say Tier 1/3; §IV-C says Tier 2. Deleting §IV-C's tool-misuse clause is a four-word fix and
   fits in current slack; changing the mapping instead means a documented v2 and a re-run.
   Until this is decided, the C3 experiment has no defined expected result.
2. **M8 — read RAIL Guard and cite it.** Three months older than this submission, owns the
   phrase "evaluation-to-remediation gap," and is distinguishable on every structural axis
   that matters. One sentence in §II-A. Do this regardless of the page budget.
3. **M7 — use the shallow-alignment result in §V-C.** The s3/s4 creep split is currently an
   unexplained 50%; Qi et al. explains it. Nearly word-neutral if it replaces the existing
   "necessary, not sufficient" sentence, and it is the strongest citation available to the
   paper.
4. **P2 — cut Figure 1.** Still the precondition for everything additive, and the new items
   have made the queue longer: C1, M7's Table I half, M8, D13 all want space.
5. **C1 — own the by-construction objection**, armed with the tick-20 off-ladder re-breach.

**Then the experiment.** C3's tool-misuse cell is still the right one to run, and Copilot
reached the same conclusion independently, but only after D12 decides what it is testing —
"Tier 2 repairs the mechanism" or "Tier 1 mis-selects and escalation recovers." The second
would exercise escalation, which no run has ever done.

**Deferred by choice:** C3 + P1, the abstract rewrite, still the other block-level space
source.

**Worth repeating:** of the eight items closed so far, none required an experiment, and the
two most recent — D12 and M7 — were found by reading the paper against its own mapping table
and against the literature. Neither needed compute either.
