...
**D12 was closed in `9d9f95e`** and **M8 in `0999127`**, both the same day they were found.
...
M8 is closed and the slack it used is gone: three filler words now render 7 pages. Everything
remaining is either blocked on space or costs compute.

1. **M7 — rewrite §V-C's creep sentence.** The last substantive edit that does not need space,
   because it replaces rather than adds. "Correct diagnosis is necessary, not sufficient"
   becomes the mechanical account: Tier 1 did not hold on `s3`, and creep is the only mode in
   the mapping table with no `escalation_tier`, so the cycle had nowhere to go and timed out.
   Two supports, Qi et al. and your own table, and roughly the same word count.
2. **P2 — cut Figure 1.** Now gating four separate things: C1, M7's Table I half, D13's
   citation, and AgentTether's `\bibitem`. Five boxes and four arrows restating the adjacent
   sentence, worth a third to half a column.
3. **C1 — own the by-construction objection**, armed with the tick-20 off-ladder re-breach.
   The strongest remaining edit in the document.
4. **D13 + AgentTether — the bibliography pass**, once P2 lands. Cite Luo et al. for Tier 3's
   uncited regression claim, and add AgentTether to §II-A's trajectory-level bundle.
5. **The experiment.** C3's tool-misuse cell now has a defined expectation under the frozen
   mapping — Tier 1 mis-selects, Tier 3 escalation recovers — and would exercise escalation for
   the first time. M1's seeds remain the answer to the stated principal limitation. Measured
   cost either way: 200 episodes and ~31M Cerebras tokens per run, 2--6 hours, every condition
   running the full 40 ticks regardless of exit tick.

**Deferred by choice:** C3 + P1, the abstract rewrite, the other block-level space source.

**Ten items closed, none of which required an experiment.** C2, D10, D11, D12 and M7's
supporting evidence came from files already on disk; M6, P4, D9, P8 and M5 from reading the
paper against itself; M8 from four abstracts. Two of those literature checks reversed my own
earlier claims — the steering "gap" in P9 and the forgetting percentage in D13 — which is the
argument for verifying before citing rather than after.
