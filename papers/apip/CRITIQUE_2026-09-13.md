...
**Addressed since this round:** **D9** and **P8** in full plus the conclusion half of **M5**
(in `1c82c15`); **M6** and **P4** in full (in `fd14b47`); and **C2** in full plus half of
**D10** (in `0a0682b` and `afbc902`) — from the existing seed-13 run record, with no new
experiment. **D11** is new, found while reading those records. Items below carry their state at the time of
...
| --- | --- | --- | --- |
| Critical | C1–C4 | 4 | 3 |
| Major | M1–M6 | 6 | 5 (M5 half-done) |
| Moderate | D1–D11 | 11 | 9 (D10 half) |
| Minor | P1–P8 | 8 | 6 |
...
*Status: **half closed in `0a0682b`.** The caption now defines Det./Act/Exit as ticks and TTR
...
*Status: **new**, from reading the run manifests.*
...
| **D11** | **(fits)** Hedge §V-E: say the contract and mapping table are content-hashed and the model is pinned by provider and name, since hosted endpoints expose no digest. Separately, and more usefully, capture whatever version metadata the API returns into the manifest from the next run onward. |
...
Reordered again after C2 closed from the run records. The standing constraint is unchanged:
roughly 20 words of slack, and only block-level removal frees more.

1. **D10's second half + D11 — one pass, both fit.** Say a tick is five episodes and give its
   relation to §IV-D's day-denominated windows; hedge §V-E's version-pinning claim to match
   what the manifest actually pins. Two sentences total, no space problem, and D11 wants doing
   before any new runs so the fix and the capture land together.
2. **Bullet 4's hedge — finish M5.** One word, word-neutral, still outstanding.
3. **P2 — cut Figure 1.** Still the only unblocked space source, and still what gates
   everything additive. Worth doing before C1 rather than after.
4. **C1 — own the by-construction objection in §V-B.** Now the strongest available edit after
   the free ones: the tick-20 off-ladder re-breach turns the paper's most serious objection
   into a result. Needs step 3's space.
5. **M1 — seeds on the τ²-bench drift cell.** Measured cost from the seed-13 records: 200
   episodes and ~31M Cerebras tokens per run, 2--6 hours wall clock, every condition running
   the full 40 ticks regardless of exit tick. Four runs (B2 + APIP × two new seeds) is ~124M
   tokens; the full sixteen-run sweep is ~500M and likely beyond the available credit.

**Still blocked on a decision rather than data:** C3 + P1, the abstract rewrite, deferred by
choice and still the other block-level space source.

**Note on what C2 cost:** nothing. The four per-cycle TTRs had been sitting in
`records.json` since 2026-09-02. Before scheduling compute for any remaining item, it is worth
asking the same question — whether the run records already answer it. D10's tick unit came
out of the same files an hour later.
