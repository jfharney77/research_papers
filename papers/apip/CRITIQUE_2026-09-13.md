...
**C5 is new and urgent** — an unreported non-reproducing seed that ships inside the released
artifact. **D12 was closed in `9d9f95e`** and **M8 in `0999127`**, both the same day they were
found.
...
*Status: **new**, found while planning the M1 runs. **This is the most urgent item in the
document.***
...
| **C5** | **(needs space — ~35 words; makes P2 mandatory rather than optional)** Report seed 10 in §V-B using the drafted replacement above, and settle which three runs the "reproduced across three seeds" claim refers to. Claim the upside while you are there: under the final contract, detection fired at tick 17 every time it fired. If tonight's runs land, update the counts to match. |
...
**C5 changes the shape of the endgame.** It is an integrity fix, it needs about 35 words, and
there are none — so P2 is no longer an improvement that would be nice to make. It is the
precondition for a disclosure the artifact release has already made mandatory.

1. **P2 — cut Figure 1, then C5.** Do these as one move. Figure 1 is five boxes and four
   arrows restating the adjacent sentence; its caption content moves into §IV's opening
   paragraph, and the reclaimed third-to-half column pays for C5 with room to spare. If cutting
   the figure is unacceptable, C3 + P1 (the abstract rewrite) is the only other block-level
   source and becomes the plan instead.
2. **The experiment**, tonight, since runs take 2--6 hours and there is no later window. Scope
   it to **B2 + APIP on three new pre-specified seeds** — six runs, ~186M Cerebras tokens —
   because roughly a quarter to a third of seeds produce no phenomenon at all and two seeds
   could yield one usable run. Kill nulls early: if `policy_citation_error_rate` is still 0.0
   at tick 20, that run is a seed-10 and half its tokens can be saved. Same frozen artifacts,
   same backbone, seed-scoped output paths, seeds pre-specified and all reported.
3. **C1** — own the by-construction objection, armed with the tick-20 off-ladder re-breach.
   First in the queue behind C5 once P2 has freed the column.
4. **M7's citation, D13, AgentTether** — the bibliography pass, once there is room for
   `\bibitem`s.
5. **M7's Table I half** — a design change against a frozen mapping; realistically 12-page.

**What tonight's runs change.** If the new seeds manifest, C5's sentence reports a stronger
ratio and M1 improves from a trajectory to a rate. If they do not, C5's sentence is still
required and the honest count is worse. Either way the disclosure is not contingent on the
experiment — which is the argument for doing P2 and C5 first, tonight, before the runs
finish.

**Twelve items closed, none of which required an experiment** — and C5, the most urgent item
in this document, was found the same way: by reading the run directory before spending money
on it.
