...
**D12 was closed in `9d9f95e`**, the same day it was found.
...
D12 is closed and bought back a little room: 20 filler words render 6 pages again, where 5 had
spilled to 7. That is enough for one short sentence, not for a paragraph.

1. **M8 — read RAIL Guard, then cite it in §II-A.** One sentence, and it now fits without
   cutting anything. Three months older than this submission, owns the phrase
   "evaluation-to-remediation gap," distinguishable on every structural axis that matters:
   dimension-level content remediation, no tier hierarchy, no behavioral contract, no bounded
   window. Do this before submission regardless of everything else.
2. **M7 — rewrite §V-C's creep sentence.** Nearly word-neutral if it replaces "correct
   diagnosis is necessary, not sufficient," and it now has two supports rather than one: Qi et
   al. on shallow alignment, and the mapping table's own missing escalation tier for creep,
   which explains the `s3` timeout mechanically.
3. **P2 — cut Figure 1.** Still the precondition for anything paragraph-sized, and the queue
   behind it is still C1, M7's Table I half, and D13.
4. **C1 — own the by-construction objection**, armed with the tick-20 off-ladder re-breach.
5. **M1 or C3 — the experiment.** C3's tool-misuse cell now has a defined expected result
   (Tier 1 mis-selects, Tier 3 escalation recovers) and would exercise escalation for the first
   time. M1's seeds remain the answer to the paper's stated principal limitation. Both cost
   compute rather than page space.

**Deferred by choice:** C3 + P1, the abstract rewrite, still the other block-level space
source.

**Nine items closed, none of which required an experiment.** C2, D10, D11, D12 and M7's
evidence all came from files already on disk — run records, manifests, and the frozen mapping
table — or from reading the paper against itself. The pattern is worth keeping: ask what the
artifacts already answer before scheduling compute.
