...
*Status: **closed in `a907edf`.** §V-B now reads "durable exit — envelope held across a
pre-specified post-intervention window, 11 ticks here — as the outcome measure." The ChatDev
twelve-tick figure did not fit and is not stated.*
...
| **M11** | *Done in `a907edf`.* State the post-intervention observation requirement for `resolved` and qualify durability as durability over that pre-specified horizon. The numbers already exist — 11 ticks for the τ² cell, 12 for ChatDev creep; this surfaces them as a criterion rather than an accident. |
