...
*Status: **new (ChatGPT pass, 2026-09-14), open.***
...
*Status: **new (ChatGPT pass), open. Verified against the run records.***
...
*Status: **new (ChatGPT pass), open. Verified.***
...
**Re-measured after `d53f311` (P2, Figure 1 cut): slack is about 39 filler words** — 39
renders 6 pages, 40 renders 7. And `\footnotesize` references were re-tested against the
6-page build: **still 7 pages.** Cutting the figure did not buy enough to reverse the
`\scriptsize` bibliography, so P3 remains open and blocked. The 39 words fund the M9/M10/
M11/D14/D15 wording pass and nothing larger.
...
| **M10** | **(fits — a rewording)** Define tiers primarily by intervention locus, and say cost and rollback difficulty generally increase rather than define the ordering. Subsumes D13's tier-span concern without an experiment. |
| **M11** | **(fits)** State the post-intervention observation requirement for `resolved` and qualify durability as durability over that pre-specified horizon. The numbers already exist — 11 ticks for the τ² cell, 12 for ChatDev creep; this surfaces them as a criterion rather than an accident. |
| **D14** | **(fits — word-neutral if it replaces nearby tier-preview prose)** One sentence before Table I defining Tier 1/2/3 at first operational use, cross-referencing §IV-C. |
| **D15** | **(fits — a rewording)** Rename the 10-tick gap from "cost of diagnosis" to "protocol overhead under the configured re-attribution cadence," and flag cadence sensitivity as future work. No run required unless the paper wants a cadence-optimality claim. |
...
| **P3** | **(blocked, measured)** P2 was not enough: with Figure 1 gone, `\footnotesize` references still render 7 pages. Restoring them needs a second block-level cut — Table I's rows or bibliography entries. Leave the bibliography at `\scriptsize` unless the venue forbids it. |
