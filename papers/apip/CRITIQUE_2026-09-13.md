...
**Addressed since this round, in `1c82c15`** (the conclusion pass): **D9** and **P8** in
full, and the conclusion half of **M5**. Items below carry their state at the time of
...
**Hard constraint on every remedy below: the paper has about 25 words of slack.** Measured
by appending filler to the current 6-page build — 20 filler words still renders 6 pages, 30
renders 7. So no remedy that adds more than a sentence can be applied until space is freed
first, and the space-freeing items (P1, P2) are therefore prerequisites rather than
cosmetics. This reorders the short list at the end.
...
| --- | --- | --- | --- |
| Critical | C1–C4 | 4 | 4 |
| Major | M1–M6 | 6 | 6 (M5 half-done) |
| Moderate | D1–D10 | 10 | 9 |
| Minor | P1–P8 | 8 | 7 |
...
*Status: **conclusion half addressed in `1c82c15`**. The §VI-A framing and contribution
bullet 4 are still open.*
...
Measured slack at the current build: **about 25 words.** Twenty filler words appended to the
6-page build still renders 6 pages; thirty renders 7. So the page budget is not merely tight,
it is effectively closed — every remaining remedy that adds prose is blocked behind P1 or P2.
Worth re-measuring after each edit.
...
| **M5** | *(b) done in `1c82c15`.* Remaining: (a) **(needs space)** rewrite the §VI-A framing as a *preregistration failure* honestly reported: the environment was selected before the architectural precondition was understood, and the result is a constraint on future environment choice, not evidence about the mechanism. (b) **Fix the conclusion's last sentence or cut it.** It is currently the least hedged statement of the claim in the paper and the most prominent. At minimum restore "hypothesized" and "plausibly" to match §VI-A; The conclusion half is done. Also outstanding and **(fits)**: hedge contribution bullet 4 in §I, now the boldest unhedged statement of the claim in the paper — one word, word-neutral. |
...
Reordered after the conclusion pass and the slack measurement. The constraint that governs
everything: **~25 words of slack**, so the space-freeing edits are no longer cosmetic
cleanup — they are the gate on every substantive fix.

1. **C3 + P1 — rewrite the abstract.** One edit, and it is the right next move for three
   reasons: C3 is a critical item, it needs no data you do not have, and it *frees roughly 35
   words*, which is what funds C1 and M2. Align the claims with what the evaluation covers
   (two categories attributed, one abstention, one uninjected; the mesh result as a scoping
   observation) and land under 150 words while you are in there. Ride **bullet 4's** one-word
   hedge along with it — same section, fits in current slack, finishes M5's prominence
   problem.
2. **C1 — name the by-construction objection in §V-B.** Roughly two sentences, so it needs
   step 1's space first. This is the most serious item in the document that requires nothing
   external, and owning it costs far less than being caught by it.
3. **C2 + D10 — one pass over Table II.** Blocked on data: B2's four per-cycle TTRs live in
   the τ²-bench run records, which are not in this repo, and the tick unit is a fact only you
   have. Everything else in the pass — the row format, the column definitions, splitting the
   two meanings of `--` — can be drafted now against placeholders.
4. **M2 — cite SPC and the drift-detection literature.** The cheapest novelty defense
   available and the most expensive in space: two sentences plus four bibliography entries at
   `\scriptsize`. Realistically this needs P2 (cutting Figure 1), not just P1.
5. **M1 — three seeds on the drift cell.** The only item here that needs compute, and still
   the highest value-per-compute available, since it converts the signature result from a
   trajectory into a rate.

**P2 (cut Figure 1) is the swing item.** It is listed as minor, but it is what makes M2, the
§VI-A reframe, D2's false-alarm table, and any other prose addition possible at all — and it
relieves P3 into the bargain. If more than one of those matters to you, do P2 early rather
than last.
