...
The collaborator's compression pass reached 6 pages; restoring two dropped citations put it
back to 7. Measured: those two `\bibitem`s *are* the overflow, and prose cuts do not move the
boundary because the last page break is set by where the bibliography ends.

1. **C6 — rewrite §V-E's non-manifestation sentence.** Urgent and non-optional: as written, the
   paper states something the evening's fp8 run disproves, and the runs are released. It is a
   rewrite rather than an addition, so it fits. Do it once `outcome.json` lands so the sentence
   can also say whether the detector fired.
2. **The wording pass — M9, M10, M11, D14, D15.** Four of the five are swaps rather than
   additions, so they fit at 7 pages and will still fit at 6. Together they repair the
   conceptual spine: what a tier is, defining it before Table I uses it, not calling a cadence
   hyperparameter "the cost of diagnosis," and saying "durable" means 11 observed ticks.
3. **The page cut — one `\bibitem` or one block.** Figure 1 remains the obvious candidate and
   survived the compression pass untouched.
4. **D13, M7's citation, AgentTether** — only if the cut frees more than it costs.

**Free, five minutes each:** P5, P6, P7. **Worth a check before submission:** the venue's
minimum-font rules, since both the bibliography and Table II are now `\scriptsize` (P3).

**Out of reach:** C3, C4, M2, M3, M4, D2--D7 — experiments or authoring, not edits.
