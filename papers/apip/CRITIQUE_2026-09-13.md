...
Everything that could be done without page space has now been done. **The remaining agenda is
P2 and what it unblocks, plus the experiments.**

1. **P2 — cut Figure 1.** No longer one item among several: it gates *five*. Five boxes and
   four arrows restating the adjacent sentence, with a caption carrying all the actual
   information. Move the caption content into §IV's opening paragraph and spend the reclaimed
   third-to-half column on the queue below.

   Behind it, in value order:
   - **C1** — own the by-construction objection in §V-B, armed with the tick-20 off-ladder
     re-breach. The strongest remaining edit in the document.
   - **M7's citation** — Qi et al., so §V-C says why the missing escalation tier is the wrong
     default rather than merely reporting that it is missing.
   - **D13** — Luo et al. (arXiv:2308.08747) for Tier 3's uncited regression claim, plus the
     clause distinguishing a LoRA adapter from full fine-tuning.
   - **AgentTether** — one key added to §II-A's existing trajectory-level `\cite` bundle.
   - **M7's Table I half** — realistically a 12-page item, since it is a design change against
     a frozen mapping.

2. **The experiments**, which cost compute rather than page space and so compete with nothing
   above. **C3's tool-misuse cell** now has a defined expectation under the frozen mapping —
   Tier 1 mis-selects, Tier 3 escalation recovers — and would exercise escalation for the first
   time in any run. **M1's seeds** remain the answer to the stated principal limitation.
   Measured cost either way: 200 episodes and ~31M Cerebras tokens per run, 2--6 hours wall
   clock, every condition running the full 40 ticks regardless of exit tick.

3. **C3 + P1 — the abstract rewrite**, deferred by choice and still the only block-level space
   source other than Figure 1. If P2 turns out to be unacceptable, this is the plan rather than
   the fallback.

**Eleven items closed, none of which required an experiment.** The sources were files already
on disk — run records, manifests, the frozen mapping table — the paper read against itself, and
four abstracts. Two literature checks reversed claims made earlier in the same session, which
is the argument for verifying before citing rather than after.
