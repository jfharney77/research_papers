...
*Status: **addressed in `0e161d3` (2026-09-15 evening), at the cost of a seventh page.*** §V-D now
adds: "The injected modes are also structurally distinct, so the funnel was never asked to
separate a confusable pair. The hard case is a stale tool schema, which the precedence rule of
§III routes to drift rather than tool misuse; testing that rule against an injector that
produces it remains future work."

Three things that sentence does. It concedes the circularity objection before a reviewer
raises it. It names the specific confusable pair rather than gesturing at one. And it points
at the M9 precedence rule as the thing that *decides* the case, which converts an unanswered
objection into a stated, testable commitment — the rule has an answer, it has simply never been
tested against an injector that produces the case. (C3 records why: the tool-misuse injector is
not implemented.)

**Cost: the paper is back to 7 pages.** The author accepted this knowingly and will find cut
candidates. Do not silently delete this sentence to recover the page — C4 is Critical, and the
remedy is ~48 words.
...
*Status: **open, and stronger than when written.** "Unused" was a reading of the paper. It is
now a measured fact about the artifact.*

**Every APIP run has `check_ins: []` and `kirkpatrick_trajectory: []`.** Checked across all
twelve APIP runs in `runs/` — chatdev s0--s4, the mock cells, the τ² cells. Not one check-in
was ever recorded, and the Kirkpatrick trajectory is empty in every `outcome.json`.

So §IV-D's check-in cadence ("Check-ins occur at 25\%, 50\%, and 75\% of the window. Each
reviews performance against the recovery trajectory at multiple Kirkpatrick levels, so that an
intervention improving surface metrics without Level 3/4 change is flagged as potentially
superficial") describes machinery that produced **no data in any evaluated run**.

**This does not make the paper wrong, but it narrows what it may claim.** §IV-D is a protocol
*design* statement and is fine as such. The two places that overreach:

- §IV-F said `kirkpatrick_levels` "captures check-in data at every level" — present tense about
  a field that is empty everywhere. Fixed: that sentence now cites `regression_ticks` and
  `exit_outcome`, which do carry data and which are what *actually* separated superficial from
  durable improvement in the results (undiagnosed: four cycles, three regressions).
- The **conclusion** said "It uses Kirkpatrick's model to separate durable recovery from
  superficial improvement." **Fixed:** it now "adapts Kirkpatrick's model to distinguish
  durable recovery from superficial improvement, and scores exit against the contract envelope
  rather than the intervention log." Kirkpatrick keeps its framing role; the mechanism claim
  now names what actually produced the result. Stronger as well as truer — re-measuring
  instead of trusting the ticket is what §V-B shows catching a partial Tier 2 refresh.

**What remains under D1** is only Table I's per-mode level assignments, which are unused but
harmless, and contribution (3)'s "evaluated through Kirkpatrick's framework", which is
borderline. Neither is a discovery risk the way the conclusion was.

A reviewer who greps the released runs for `kirkpatrick` finds two empty arrays. The cheapest
honest repair is to present Kirkpatrick as the *framing* that motivates the durability
criterion rather than as an instrument that was read off.
...
*Status: **closed by deletion, 2026-09-15 evening.** §IV-C's Tier 3 sentence now reads "covers
fine-tuning, DPO, or replacement when lower tiers are exhausted or failure is embedded in model
weights." Both halves of the objection lose their target: the uncited regression-risk claim is
gone, and so is "highest cost", which also removes the last per-tier cost claim that M10 made
redundant. 16 words saved, and no citation had to be added. The note below is kept because the
sourcing caution stays useful if Tier 3's risk profile is ever argued in the 12-page version.*
...
| 7 | **Durable exit is defined by protocol parameters** — two consecutive in-envelope ticks, then a ten-tick observation window — **not by observed tick counts**, and the definition lives in §IV-E with the other exit states. | Quoting "11 ticks" invites the right-censoring objection, which is what the first attempt at M11 did. In §V-B it read as an evaluation choice; in §IV-E it is self-evidently pre-specified. | **Yes** — `a907edf` quoted the counts; fixed in `8b77f2e`; moved to §IV-E later. |
| 13 | **§IV-E states *Regressed* as "an envelope restored and then breached again".** | That is the scored criterion behind Table II's three regression events. "An apparent recovery that did not hold" is the interpretation, not the test. | **Yes** — lost 2026-09-15 evening when the em-dash pair carrying it was removed (P10's predicted failure). |
| 14 | **§IV-C says Tier 1 is the first response to "much input brittleness".** | Table I routes input brittleness to Tier 1 **or** Tier 3. Without the hedge §IV-C contradicts the table, the same shape as D12. | **Yes** — hedge dropped 2026-09-15 evening, restored the same day. |
| 15 | **§IV-E describes all five exit states, including `resolved`.** | The success state was described only in §V-B, so the framework section never said what success meant — the fifth first-use-before-definition (D14b). | No |
...
| 12 | **Adversarial subversion is out of the paper, and so are `greenblatt2023`, `bhatt2025`, `terekhov2025`.** *(Inverted 2026-09-15 evening.)* | The author's original instinct: a sixth category never mentioned again. It was briefly kept in §II-A only to stop three `\bibitem`s being orphaned; when 7 pages became fatal, dropping paragraph and references together was the single biggest lever. Restoring the paragraph without the bibitems orphans three entries, and LaTeX will not warn — the bibliography is manual. | n/a |
...
**The paper is at 6 pages as of 2026-09-15 evening, and the budget is spent.** Getting there
from 7 took exactly three cuts, about 104 words, the figure bisected rather than estimated:
the §II-A adversarial declaration plus its three `\bibitem`s, §VI-A's candidate-mechanisms
list, and §VI-A's Kim et al. task detail.

**What was measured NOT to work, so nobody repeats it:** cutting any one of the three open
problems (they sit after the spill, so the lines that overflow come later in the document);
dropping five bibliography entries alone; reverting the contributions list to inline numbering,
which bought a full page in the afternoon and nothing by evening; D1's Table I Kirkpatrick
levels. **Any addition from here spills to 7 — cut an equal amount in the same edit.** Measured slack: **1 word** at §V-B,
