...
(in `1c82c15`), and **M6** and **P4** in full (in `fd14b47`). Items below carry their state at the time of
...
Reordered twice now: once after the conclusion pass, again after the M6/P4 sweep showed that
**word-level deletion does not free page space.** With the abstract rewrite (C3 + P1) tabled,
every additive remedy is gated behind one block-level removal.

1. **P2 — cut Figure 1.** Now the critical path, not a minor cleanup. It is the only unblocked
   space source left: five boxes and four arrows restating the adjacent sentence, worth a third
   to half a column. Nothing else additive can happen until this or the abstract gives way.
   Move its caption content into §IV's opening paragraph, where it is already half-stated.
2. **Bullet 4's hedge — finish M5.** One word, word-neutral, fits in current slack, needs
   nothing from step 1. With the conclusion repaired this is the boldest unhedged statement of
   the mesh claim left in the paper, and it sits in the contribution list where reviewers look
   first.
3. **C1 — own the by-construction objection in §V-B.** Two sentences, so it needs step 1
   first. The most serious item that requires nothing external. M6's fix sharpened the need
   rather than reducing it: the paper no longer claims B2 represents industry, which makes the
   question "then why is B2 the comparison?" more pressing, and C1 is the answer.
4. **D1 — delete the Kirkpatrick levels from Table I's cells.** Block-level, so it genuinely
   frees space, and it closes a moderate item. Keep the framework where it does real work (the
   Level 1/2 vs. 3/4 durability distinction in §II-C).
5. **M1 — three seeds on the τ²-bench drift cell.** The only item that costs compute rather
   than page space, so it runs in parallel with all of the above and competes with none of it.

**Still blocked on facts only the authors have:** C2 (B2's four per-cycle TTRs, in the
τ²-bench run records, which are not in this repo) and D10's tick definition. That pass remains
the highest-value edit in the document and can be drafted against placeholders at any time.

**Deferred by choice:** C3 + P1, the abstract rewrite. Worth remembering that it is the other
block-level space source — 35 words out of the abstract is a real gain, unlike 35 words out of
body prose — so if P2 turns out to be unacceptable, this is the fallback rather than a
parallel option.
