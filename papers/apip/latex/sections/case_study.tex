...
measurements. The scenario is not left hypothetical: the
$\tau^2$-bench drift experiment in Section~\ref{subsec:eval-tau} reproduces
this failure mode end-to-end with deterministic ground truth---a policy
corpus that goes stale while the world's ground truth moves---and measures
what this section can only narrate.
...
Each of these claims is testable, and Section~\ref{sec:simulation} tests them:
the ad-hoc responder of Section~\ref{subsec:degradation-event} becomes the
fixed-Tier-1 baseline condition (B2), whose patch-then-regress dynamic is
measured rather than narrated, and Aria's trigger metric---the policy citation
error rate---is the drift experiment's judge-scored detection metric.
