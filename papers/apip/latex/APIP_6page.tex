...
(Fig.~\ref{fig:lifecycle}): Trigger, Cause Attribution, Intervention Planning,
...
This composes with, rather than duplicates, automated failure attribution,
which localizes the responsible agent or step within a single failed trajectory
\cite{zhu2026, ma2025attribution, zhang2025, agenttrace2026}. Phase 2 operates
one level of aggregation above, classifying a window-level degradation into a
...
diagnosing the cause---the gap mandatory taxonomy classification closes.
...
warnings: no improvement at the first check-in escalates the tier rather than
waiting for expiry.
...
Fig.~\ref{fig:lifecycle}. Three carry argumentative weight. \emph{Regressed}
---envelope restored, then breached again---is the signature of a
tier-mismatched intervention that suppressed symptoms, and naming it as an
outcome is what makes that failure visible rather than counted as a success.
\emph{Recalibrated} records that attribution concluded the \emph{environment}
legitimately moved, so the contract is re-baselined rather than the agent
treated; we deliberately do not score it as a resolution, since conflating the
two would let re-baselining masquerade as remediation. \emph{Retired} closes
the agent out when it cannot be returned to contract at acceptable cost. Window
\emph{extension} is an in-protocol check-in decision, not an exit, so every
APIP terminates in exactly one of the five states and the record joins the
...
work: \code{behavioral\_contract\_ref} forces a contract to exist at deployment
time, without which trigger and exit cannot be operationalized;
\code{kirkpatrick\_levels} captures check-in data at every level, enabling
...
linkage, letting an auditor verify which attribution rules and environment
versions were actually in force and so converting pre-registration from
assertion into checkable property; and \code{mesh\_context}
(Section~\ref{sec:mesh}) supports multi-agent attribution.
